...
model_type = 'LSTM' # 'LSTM' 'BILSTM' 'GRU' 
model_name = 'LSTM.h5' # LSTM.h5 BILSTM.h5
batch_size = 1
...
def CreateModel():
    if model_type == 'LSTM':
        ...
        # Hidden layer
        model.add(Bidirectional(LSTM(units = number_neural)))
        model.add(Dense(1))
        #Compile model
        ...
        # Hidden layer
        model.add(GRU(units = number_neural))                 
        model.add(Dropout(0.2))
...
# %% Read Dataset
df = pd.read_csv('data/data.csv', parse_dates=True, index_col=Date_Column)
# %% Data Preprocessing
# Scaling between 0, 1
...
def accuracy(text, forecast, actual):
    mse = tf.keras.losses.mean_squared_error(forecast, actual).numpy()
    mape = tf.keras.losses.mean_absolute_percentage_error(forecast, actual).numpy()
    mae = tf.keras.losses.mean_absolute_error(forecast, actual).numpy()
    msle = tf.keras.losses.mean_squared_logarithmic_error(forecast, actual).numpy()
    # mape = 100 * np.mean(np.abs(actual - forecast)/np.abs(forecast))  # MAPE
    me = np.mean(forecast - actual)             # ME
    # mae = np.mean(np.abs(forecast - actual))    # MAE
    mpe = 100 * np.mean((forecast - actual)/forecast)   # MPE
    ...
    return pd.DataFrame({ '': [text], 'ME':[me], 'RMSE':[rmse], 'MAE': [mae], 'MPE': [mpe],  'MAPE':[mape], 'ACF1':[acf1] })
print(accuracy('Train Score:',y_train[0], X_train_Predict[:,0]))
print(accuracy('Valid Score:', y_valid[0], X_valid_Predict[:,0]))
print(accuracy('Test Score:', y_test[0], X_test_Predict[:,0]))
